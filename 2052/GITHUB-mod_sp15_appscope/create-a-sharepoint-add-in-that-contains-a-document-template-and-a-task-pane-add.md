...
Use Visual Studio to develop an Office 外接程序 that appears in a document that is opened from a SharePoint 外接程序.
You can create a SharePoint 外接程序 that includes a document template (for example, an expense report). The document can include a task pane add-in that interacts with SharePoint data. For example, users can populate fields of an invoice by using data from the Business Connectivity Services (BCS) or create an expense report by selecting an expense category from a SharePoint list.
  
    
    

This walkthrough shows you how to create a SharePoint 外接程序 that includes an Excel workbook. The Excel workbook contains a task pane add-in that uses the REST interface provided by SharePoint 2013 to populate a drop-down list box with SharePoint date in the task pane add-in.
## Prerequisites
...
- A SharePoint development environment:
  ...
  - To develop SharePoint 外接程序 that target SharePoint in Office 365, see  [How to: Set up an environment for developing SharePoint Add-ins on Office 365](http://msdn.microsoft.com/en-us/library/office/apps/fp161179%28v=office.15%29).
    
  
  - To develop SharePoint 外接程序 that target an on-premises installation of SharePoint, see  [How to: Set up an on-premises development environment for SharePoint Add-ins](http://msdn.microsoft.com/en-us/library/office/apps/fp179923%28v=office.15%29).
...
## Create a SharePoint 外接程序 project in Visual Studio
<a name="Create"> </a>
...
6. In the **How do you want to host your SharePoint 外接程序?** drop-down list, choose **SharePoint-hosted** and then choose **Next**.
    
    > [!注释]
      > This scenario works only with the SharePoint-hosted and provider-hosted options presented in the **How do you want to host your SharePoint 外接程序?** drop-down list.
...
Next, add an Office 外接程序 to the project. You can add any type of add-in that you want. For this walkthrough, we will add a task pane add-in.
...
6. In the **Would you like your Office 外接程序 to appear in a new document or in an existing document?** page, choose **Create a new document and insert my add-in**, and then choose **Finish**.
...
12. In the manifest designer, set the value of the **Start page** value to~appWebUrl/Lists/MyDocumentLibrary. This converts to a value of OfficeEnabledAddin/Lists/MyDocumentLibrary.
    ...
    > [!注释]
      > This URL refers to the document library. You must use the ~appWebUrl token at the beginning of any URL in your Office 外接程序 manifest that refers to items within the add-in web. For more information about URL tokens in a SharePoint 外接程序 project, see  [SharePoint 外接程序中的 URL 字符串和标记](url-strings-and-tokens-in-sharepoint-add-ins.md). 
...
You can debug your task pane add-in by starting the document or by starting the SharePoint 外接程序, and then opening a document from the document library.
...
When you are ready to package your add-in for publishing, open the **Publish Office and SharePoint 外接程序** wizard.
  
    
    

- In **Solution Explorer**, open the shortcut menu for the SharePoint 外接程序 project, and then choose **Publish**.
    
    The **Publish Office and SharePoint 外接程序** wizard appears. For more information, see [使用 Visual Studio 发布 SharePoint 外接程序](publish-sharepoint-add-ins-by-using-visual-studio.md).
