...
Use Visual Studio to develop an Office 增益集 that appears in a document that is opened from a SharePoint 增益集.
You can create a SharePoint 增益集 that includes a document template (for example, an expense report). The document can include a task pane add-in that interacts with SharePoint data. For example, users can populate fields of an invoice by using data from the Business Connectivity Services (BCS) or create an expense report by selecting an expense category from a SharePoint list.
  
    
    

This walkthrough shows you how to create a SharePoint 增益集 that includes an Excel workbook. The Excel workbook contains a task pane add-in that uses the REST interface provided by SharePoint 2013 to populate a drop-down list box with SharePoint date in the task pane add-in.
## Prerequisites
...
- A SharePoint development environment:
  ...
  - To develop SharePoint 增益集 that target SharePoint in Office 365, see  [How to: Set up an environment for developing SharePoint Add-ins on Office 365](http://msdn.microsoft.com/en-us/library/office/apps/fp161179%28v=office.15%29).
    
  
  - To develop SharePoint 增益集 that target an on-premises installation of SharePoint, see  [How to: Set up an on-premises development environment for SharePoint Add-ins](http://msdn.microsoft.com/en-us/library/office/apps/fp179923%28v=office.15%29).
...
## Create a SharePoint 增益集 project in Visual Studio
<a name="Create"> </a>
...
6. In the **How do you want to host your SharePoint 增益集?** drop-down list, choose **SharePoint-hosted** and then choose **Next**.
    
    > [!注意事項]
      > This scenario works only with the SharePoint-hosted and provider-hosted options presented in the **How do you want to host your SharePoint 增益集?** drop-down list.
...
Next, add an Office 增益集 to the project. You can add any type of add-in that you want. For this walkthrough, we will add a task pane add-in.
...
6. In the **Would you like your Office 增益集 to appear in a new document or in an existing document?** page, choose **Create a new document and insert my add-in**, and then choose **Finish**.
...
12. In the manifest designer, set the value of the **Start page** value to~appWebUrl/Lists/MyDocumentLibrary. This converts to a value of OfficeEnabledAddin/Lists/MyDocumentLibrary.
    ...
    > [!注意事項]
      > This URL refers to the document library. You must use the ~appWebUrl token at the beginning of any URL in your Office 增益集 manifest that refers to items within the add-in web. For more information about URL tokens in a SharePoint 增益集 project, see  [URL 字串及權杖中 SharePoint 增益集](url-strings-and-tokens-in-sharepoint-add-ins.md). 
...
-  [保存的增益集狀態與設定](http://msdn.microsoft.com/library/407df6e8-c237-4d6a-b357-3000fe3de9ff%28Office.15%29.aspx)
...
You can debug your task pane add-in by starting the document or by starting the SharePoint 增益集, and then opening a document from the document library.
...
When you are ready to package your add-in for publishing, open the **Publish Office and SharePoint 增益集** wizard.
  
    
    

- In **Solution Explorer**, open the shortcut menu for the SharePoint 增益集 project, and then choose **Publish**.
    
    The **Publish Office and SharePoint 增益集** wizard appears. For more information, see [使用 Visual Studio 發佈 SharePoint 增益集](publish-sharepoint-add-ins-by-using-visual-studio.md).
...
-  [工作窗格與內容增益集的 Office 2013](http://msdn.microsoft.com/library/baf73b23-4429-4b7f-bcb9-a99a9618ae38%28Office.15%29.aspx)
    
  
-  [Office 增益集的設計指導方針](http://msdn.microsoft.com/library/d5b2ab2e-dfc8-47c8-919c-e9c23358d70c%28Office.15%29.aspx)
    
  
-  [Office 增益集開發技術支援週期](http://msdn.microsoft.com/library/c35b4b2b-7869-4501-9f10-888c8e74c98c%28Office.15%29.aspx)
    
  
-  [發佈 Office 增益集](http://msdn.microsoft.com/library/7f3ae6a0-06e9-438c-8899-bd9f605e6d9e%28Office.15%29.aspx)
    
  
-  [瞭解 Office 的 JavaScript API](http://msdn.microsoft.com/library/01180dae-ca45-40c8-b3dd-fd2a85651c0c%28Office.15%29.aspx)
    
  
-  [Office 增益集的 XML 資訊清單](http://msdn.microsoft.com/library/4139ff24-afac-472a-af7d-9d069587ac9b%28Office.15%29.aspx)
    
  
-  [Office 增益集 API 與結構描述參考](http://msdn.microsoft.com/library/afcc2908-1e1b-4297-b554-11e6eb404804%28Office.15%29.aspx)
